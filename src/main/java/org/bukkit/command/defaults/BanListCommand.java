...
public class BanListCommand extends VanillaCommand {
	private static final List<String> BANLIST_TYPES = ImmutableList.of("ips",
			"players");
	...
	public boolean execute(CommandSender sender, String currentAlias,
		...
		OfflinePlayer[] banlist = Bukkit
				.getServer()
				.getBannedPlayers()
				.toArray(
						new OfflinePlayer[Bukkit.getServer().getBannedPlayers()
								.size()]);
		...
		sender.sendMessage("There are " + banlist.length
	...
	public List<String> tabComplete(CommandSender sender, String alias,
		...
		if (args.length == 1) {
			return StringUtil.copyPartialMatches(args[0], BANLIST_TYPES,
					new ArrayList<String>(BANLIST_TYPES.size()));
		}
