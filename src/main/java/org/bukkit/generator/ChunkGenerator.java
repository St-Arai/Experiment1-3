...
/**
 * A chunk generator is responsible for the initial shaping of an entire chunk. For example, the
 * nether chunk generator should shape netherrack and soulsand
 */
public abstract class ChunkGenerator {
  /**
   * Interface to biome data for chunk to be generated: initialized with default values for world
   * type and seed.
   * 
   * Custom generator is free to access and tailor values during generateBlockSections() or
   ...
   * <p>
   * As of 1.2, chunks are represented by a vertical array of chunk sections, each of which is 16 x
   * 16 x 16 blocks. If a section is empty (all zero), the section does not need to be supplied,
   ...
   * <pre>
     ...
     * </pre>
   * Each section (sectionID = (Y>>4)) that has blocks needs to be allocated space for the 4096
   ...
   * <pre>
     ...
     * </pre>
   * <p>
   * Note that this method should <b>never</b> attempt to get the Chunk at the passed coordinates,
   * as doing so may cause an infinite loop
   * <p>
   * Note generators that do not return block IDs above 255 should not implement this method, or
   * should have it return null (which will result in the generateBlockSections() method being
   * called).
   * 
   * @param world The world this chunk will be used for
   * @param random The random generator to use
  ...
  public byte[][] generateBlockSections(World world, Random random, int x, int z, BiomeGrid biomes) {
    ...
    return a; // Default - returns null, which drives call to generate()
  }
   ...
   * <p>
   * A null value is returned if a world should not use a fixed spawn point, and will instead
   * attempt to find one randomly.
   * 
